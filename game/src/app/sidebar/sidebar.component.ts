...
export class SidebarComponent implements OnInit {
  list: any = [
    ...
    { curr: 'BUSD', amount: 1.0, id: 14 },
    { curr: 'RGC', amount: 1.0, id: 15 },
  ...
  games = [
    ...
    },
    {
      name: 'Bustabit',
      route: '/bustabit',
      img: 'assets/images/bustabit.png',
      rtp: '95.66%',
      biggestWin: '883.18X',
    },
    {
    ...
    },
    {
      name: 'Sword',
      route: '/sword',
      img: 'assets/images/sword-g.png',
      rtp: '95.66%',
      biggestWin: '883.18X',
    },
    {
      name: 'Ring of furtune',
      route: '/ringoffortune',
      img: 'assets/images/all/4.png',
      rtp: '95.66%',
      biggestWin: '883.18X',
    },
    {
      name: 'Mines',
      route: '/mines',
      img: 'assets/images/all/3.png',
      rtp: '95.66%',
      biggestWin: '883.18X',
    },
    {
      name: 'Cave of plunder',
      route: '/caveofplunder',
      img: 'assets/images/plunder-g.png',
      rtp: '95.66%',
      biggestWin: '883.18X',
    },
  ];
  ...
  openModel1() {
    ...
    this.httpService
      .getRequest('basic/getVip', this.token)
  ...
  spinNow() {
    if (!this.httpService.loggedIn()) {
      ...
      this.token = localStorage.getItem('gAmE-t0KEN');
      this.httpService
        .postRequest('basic/luckwheel', obj, this.token)
        .subscribe((resData: any) => {
          if (resData.success == 1) {
            ...
            if (spin[random] == 1) {
              var value = 3600;
            } else if (spin[random] == 2) {
              var value = 3600 + 337.5;
            } else if (spin[random] == 3) {
              var value = 3600 + 315;
            } else if (spin[random] == 4) {
              var value = 3600 + 292.5;
            } else if (spin[random] == 5) {
              var value = 3600 + 270;
            } else if (spin[random] == 6) {
              var value = 3600 + 247.5;
            } else if (spin[random] == 7) {
              var value = 3600 + 225;
            } else if (spin[random] == 8) {
              var value = 3600 + 202.5;
            } else if (spin[random] == 9) {
              var value = 3600 + 180;
            } else if (spin[random] == 10) {
              var value = 3600 + 157.5;
            } else if (spin[random] == 11) {
              var value = 3600 + 135;
            } else if (spin[random] == 12) {
              var value = 3600 + 112.5;
            } else if (spin[random] == 13) {
              var value = 3600 + 90;
            } else if (spin[random] == 14) {
              var value = 3600 + 67.5;
            } else if (spin[random] == 15) {
              var value = 3600 + 45;
            } else if (spin[random] == 16) {
              var value = 3600 + 22.5;
            }
  ...
  spinData() {
    if (!this.httpService.loggedIn()) {
      (<HTMLInputElement>document.getElementById('spinvalue')).textContent =
        'Lucky spin 1';
    } else {
      this.token = localStorage.getItem('gAmE-t0KEN');
      this.httpService
        .getRequest('basic/spinData', this.token)
        .subscribe((resData: any) => {
          if (resData.success == 1) {
  ...
  bonusData() {
    ...
    this.httpService
      .postRequest('basic/bonus', paramsdata, this.token)
      .subscribe((resData: any) => {
        if (resData.success == 1) {
  ...
  getlocked(curr: any) {
    var data = { curr: curr };
    this.httpService
      .postRequest('basic/getlockedamt', data, this.token)
  ...
  unlockbal(curr: any) {
    ...
    this.httpService
      .postRequest('basic/unlockamt', data, this.token)
      .subscribe((resDate: any) => {
        if (resDate.success == 1) {
